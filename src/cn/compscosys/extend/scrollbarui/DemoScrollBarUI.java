package cn.compscosys.extend.scrollbarui;
...
public class DemoScrollBarUI extends BasicScrollBarUI {
	protected void configureScrollBarColors() {
		...
		// 滑道
		trackColor = new Color(252, 247, 244);//Color.black;
	...
	public void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
		...
		if (this.scrollbar.getOrientation() == JScrollBar.VERTICAL) {
			//设置画笔
			gp = new GradientPaint(0, 0, new Color(251, 241, 243, 0), trackBounds.width, 0, new Color(251, 241, 243, 0));
		}
		else if (this.scrollbar.getOrientation() == JScrollBar.HORIZONTAL) {
			gp = new GradientPaint(0, 0, new Color(251, 241, 243, 0), trackBounds.height, 0, new Color(251, 241, 243, 0));
		}
	...
	protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
		...
		// 设置把手颜色
		g.setColor(new Color(160, 166, 172));

		// 画一个圆角矩形
		// 这里面前四个参数就不多讲了，坐标和宽高
		// 后两个参数需要注意一下，是用来控制角落的圆角弧度
		//g.drawRoundRect(0, 0, 8, thumbBounds.height - 1, 8, 8);
		...
		// 填充圆角矩形
		g2.fillRoundRect(0, 0, 8, thumbBounds.height - 1, 8, 8);
	}
